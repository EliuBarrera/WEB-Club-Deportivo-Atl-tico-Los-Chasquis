...
- **Aval institucional** (`components/home/RespaldoInstitucional.tsx`):
  ...
  **Se probó y se revirtió (2026-09-08):** una variante con dos niveles
  de jerarquía (avales grandes arriba, socios chicos abajo) sobre fondo
  casi-negro, con cada logo recoloreado a naranja vía CSS `mask-image`
  (`Socio.nivel`, enum `AVAL`/`SOCIO`). Durante la verificación visual se
  detectó que varios de los 18 logos reales son insignias circulares
  casi totalmente opacas (medido con ImageMagick — ej. "Comisión
  Departamental de Juzgamiento" ~79% de opacidad promedio), así que la
  máscara los colapsaba a una mancha sólida irreconocible; se compensó
  con un rótulo de texto visible bajo cada logo, pero el resultado no
  convenció al club, que pidió volver a la versión anterior (logos a
  color sobre tarjetas blancas). Se revirtió por completo: el campo

...
// Modal de creación/edición de una cifra de confianza (Fase 9) — mismo
// patrón que DocumentoLegalModal.tsx.
export function CifraConfianzaModal({
  abierto,
  cifra,
  guardarAction,
  eliminarAction,
        ...
        >
          <div className="flex items-center justify-between">
            <h2 className="font-display text-xl font-extrabold uppercase">
              {cifra ? "Editar cifra" : "Nueva cifra"}
            </h2>
            <button
              type="button"
              aria-label="Cerrar"
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-[2fr_1fr]">
              <label className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  defaultValue={cifra?.etiqueta ?? ""}
                  className={campoClase}
                />
              </label>
              <label className="flex flex-col gap-1.5">
                <span className={labelClase}>Valor</span>
                <input
                  type="text"
                  name="valor"
                  required
                  placeholder="Ej: 45+"
                  defaultValue={cifra?.valor ?? ""}
                  className={campoClase}
                />
              </label>
            </div>
      ...
      {eliminarAction && (
        <>
          ...
          <ConfirmDialog
            ...
            descripcion={`¿Eliminar "${cifra?.etiqueta}"? Esta acción no se puede deshacer.`}
